...
int DoPatch(HANDLE hPatch, HANDLE hSource, HANDLE hDest) {
    ...
    while (patches--) {
        ...
        // read checksums
        if(!MD5Mode) {
            unsigned int patch_source_crc = 0;
            ...
            while (patch_blocks--) {
                ...
                switch (blocktype) {
                    ...
                    case 3:
                        ...
                        { unsigned int x; blocksize = ReadFile(hPatch,&x,4,&read,NULL)? x:0; }
